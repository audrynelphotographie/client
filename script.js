/*
  ...
  Handles homepage, client access, gallery viewer and downloads.
*/
...
function getClients() {
  try {
    ...
    return Array.isArray(saved) ? saved : DEFAULT_CLIENTS;
  } catch {
    return DEFAULT_CLIENTS;
  }
}

function saveActivity(type, clientId = "") {
  try {
...
function renderGalleryPage(clientId) {
  const client = getClients().find(c => c.id === clientId);
  ...
  (client.photos || []).forEach((photo, index) => {
    ...
    card.innerHTML = `
      <div class="card-cover">
        <img src="${escapeHTML(photo.standard || photo.hd || "")}" alt="Photo ${index + 1}" loading="lazy">
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#clientGrid")?.addEventListener("click", e => {
    ...
    if (view) openAccessModal(view.dataset.client);
    if (share) shareClient(share.dataset.share);
  });
